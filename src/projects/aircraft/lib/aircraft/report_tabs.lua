...
local function configSections(config)
  ...
  add(display, config, "display.absoluteRotorValues", "When true, rotor Nixies show absolute display values even if actuator output signs are negative.", "aircraft config nixie-abs true")
  add(display, config, "display.statusStrip.enabled", "Whether a reserved 3-Nixie throttle/H/T status strip is updated with rotor Nixies.", "aircraft config status-strip true <x> <y> <z> +X")
  add(display, config, "display.statusStrip.x", "Status strip anchor x coordinate. The anchor cell shows throttle/base RPM.")
  add(display, config, "display.statusStrip.y", "Status strip anchor y coordinate.")
  add(display, config, "display.statusStrip.z", "Status strip anchor z coordinate.")
  add(display, config, "display.statusStrip.axis", "Direction from throttle cell to H and T cells. The scanner reserves anchor, anchor+axis, and anchor+2*axis.")
