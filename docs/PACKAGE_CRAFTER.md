...
Crafting is intentionally conservative, but optimized after a recipe is known. `turtle.craft(limit)` reports only success or failure, not the number of items crafted. For each unknown recipe, the turtle first crafts one item, reads the output slot, records whether any remainder items were left in the grid, and saves that recipe data to `/config/package_crafter_recipes.lua` on the turtle. The cache is local to the turtle filesystem and is written with `textutils.serialize`.
